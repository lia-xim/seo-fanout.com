...
async function inspect(route, viewport) {
  ...
  check(overflow <= 1, `${route} overflows viewport by ${overflow}px at ${viewport.width}px`);
  const axe = await new AxeBuilder({ page }).analyze();
  const serious = axe.violations.filter((item) => ["serious", "critical"].includes(item.impact || ""));
  check(serious.length === 0, `${route} has serious Axe violations: ${serious.map((item) => item.id).join(", ")}`);
  ...
  results.push({ route, viewport: viewport.width, axeViolations: axe.violations.length, axeViolationIds: axe.violations.map((item) => item.id), seriousAxeViolations: serious.length, overflow, consoleErrors: errors.length, ...measured });
  await context.close();
}

const sitemap = await (await fetch(new URL("/sitemap-0.xml", baseUrl))).text();
const routes = [...sitemap.matchAll(/<loc>([^<]+)<\/loc>/g)].map(([, value]) => new URL(value).pathname);
...
const page = await context.newPage();
await page.goto(new URL("/page-inventory/", baseUrl).toString(), { waitUntil: "networkidle" });
...
check(await page.locator("#stress-grid article").count() === 6, "stress test did not render six inventory states");

await page.goto(new URL("/tool/", baseUrl).toString(), { waitUntil: "networkidle" });
await page.keyboard.press("Tab");
check(await page.locator(":focus").count() === 1, "keyboard focus is not present in the DOM");
await context.close();

const noJsContext = await browser.newContext({ javaScriptEnabled: false });
const noJsPage = await noJsContext.newPage();
await noJsPage.goto(new URL("/page-inventory/", baseUrl).toString());
check(await noJsPage.getByRole("heading", { level: 1 }).isVisible(), "worksheet main content depends on JavaScript");
await noJsPage.goto(new URL("/tool/", baseUrl).toString());
check(await noJsPage.getByRole("heading", { level: 1 }).isVisible(), "tool explanation depends on JavaScript");
